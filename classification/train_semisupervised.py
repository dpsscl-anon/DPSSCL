...
from classification.gen_data import mnist_data_print_info, cifar_data_print_info, print_data_info
...
MAKO_DATA_ROOT = os.path.join(PROJECT_ROOT, 'Data', 'task_data')
...
def train_semilifelong(model_architecture, model_hyperpara, train_hyperpara, dataset, data_type, classification_prob, useGPU=False, GPU_device=0, save_param=False, param_folder_path='saved_param', save_param_interval=100, save_graph=False, run_cnt=0, task_order=None, weak_labeler_root=None):
    print("Training function for semi-supervised lifelong learning (Mako)!")
    ...
    if 'mnist' in data_type:
        ...
        if weak_labeler_root is None:
            weak_labeler_root = os.path.join(MAKO_DATA_ROOT, 'mnist_bin')
        ...
        if weak_labeler_root is None:
            weak_labeler_root = os.path.join(MAKO_DATA_ROOT, 'cifar10_bin')
        remove_weak_labeler_modelfiles(weak_labeler_root)
    elif 'cifar100' in data_type:
        num_task, num_train, num_valid, num_test, x_dim, y_dim, y_depth = cifar_data_print_info(labeled_data, unlabeled_data, test_data_torch, True, print_info=False)
        if weak_labeler_root is None:
            weak_labeler_root = os.path.join(MAKO_DATA_ROOT, 'cifar100_bin')
        remove_weak_labeler_modelfiles(weak_labeler_root)
    else:
    ...
    for train_task_cnt, (task_for_train) in enumerate(task_training_order):
        ...
        if 'mnist' in data_type:
            labelers_transferred_task_ids, labelers_converged_epoch_ratio, snuba_iter_cnt = generate_labeler_for_one_task(task_dir=os.path.join(weak_labeler_root, task_dir_list[task_for_train]), task='mnist', data=data_for_labeler_gen, task_similarity_score=transfer_score, task_similarity_score_threshold=task_similarity_score_threshold, sample_lf_from_earlier_task_prob=sample_lf_from_earlier_task_prob, path_to_earlier_task_lf=[task_dir_list[task_training_order[a]] for a in range(train_task_cnt)], lg_min=lg_min, lg_max=lg_max, lg_keep=lg_keep, return_snuba_iter_cnt=True, unified_score=unified_score, finetune_transferred_wls=finetune_transferred_wls)
    ...
    result_summary['makoint_LEEPscore'] = transfer_score_hist
