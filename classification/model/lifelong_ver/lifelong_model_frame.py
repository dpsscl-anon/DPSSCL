...
class Lifelong_Model_Frame():
    ...
    def compute_confusion_matrix(self, sess, data_x, data_y, task_index, dropout_prob=1.0):
        def update_confusion_matrix(true_y, pred_y, confusion_matrix, num_instances_to_check):
            assert (true_y.shape[0]>=num_instances_to_check and pred_y.shape[0]>=num_instances_to_check), "Either true labels or predictions are less than the required amount"
            for cnt in range(num_instances_to_check):
                confusion_matrix[true_y[cnt], pred_y[cnt]] += 1
            return confusion_matrix

        ## rows - true labels, columns - prediction, each element - # of instances w.r.t. pairs of true labels and predictions
        task_model_index = self.find_task_model(task_index)
        num_classes = self.output_sizes[task_model_index]
        ...
        for batch_cnt in range(num_batch):
            x, y = data_x[batch_cnt*self.batch_size:(batch_cnt+1)*self.batch_size], data_y[batch_cnt*self.batch_size:(batch_cnt+1)*self.batch_size]
            pred_y = sess.run(self.pred[task_model_index], feed_dict={self.model_input: x, self.dropout_prob: dropout_prob, self.num_data_in_batch: self.batch_size})
            conf_mtx = update_confusion_matrix(y, pred_y, conf_mtx, self.batch_size)
        if num_remains > 0:
            tmp_x, tmp_y = data_x_and_y_add_dummy(data_x[-num_remains:], data_y[-num_remains:], self.batch_size)
            pred_y = sess.run(self.pred[task_model_index], feed_dict={self.model_input: tmp_x, self.dropout_prob: dropout_prob, self.num_data_in_batch: num_remains})
            conf_mtx = update_confusion_matrix(tmp_y, pred_y, conf_mtx, num_remains)
        return conf_mtx
