...
from classification.gen_data import mnist_data_print_info, cifar_data_print_info, omniglot_data_print_info, officehome_data_print_info
from classification.gen_data import officehome_incremental_data_print_info
...
from utils.utils_cl import compute_mutual_info_convNet

from classification.gen_data import print_data_info

#from classification.model.lifelong_ver.obsolete_model import LL_hybrid_DFCNN_automatic_sharing, LL_hybrid_DFCNN_automatic_sharing_ver2, LL_hybrid_DFCNN_automatic_sharing_ver3, LL_hybrid_DFCNN_automatic_sharing_ver4
from classification.model.lifelong_ver.cnn_baseline_model import LL_several_CNN_minibatch, LL_single_CNN_minibatch, LL_CNN_HPS_minibatch, LL_CNN_progressive_net, LL_CNN_tensorfactor_minibatch, LL_CNN_ChannelGatedNet, LL_CNN_APD
...
from classification.model.multitask_ver.cnn_baseline_model import MTL_several_CNN_minibatch, MTL_CNN_minibatch, MTL_CNN_HPS_minibatch, MTL_CNN_tensorfactor_minibatch, MTL_several_CNN_deconvtm_minibatch, MTL_several_CNN_deconvtm2_minibatch
from classification.model.multitask_ver.cnn_ewc_model import MTL_CNN_EWC_minibatch
from classification.model.multitask_ver.cnn_df_model import MTL_DFCNN_direct_minibatch, MTL_DFCNN_minibatch, MTL_DFCNN_tc2_minibatch, MTL_HybridDFCNN_resnet_minibatch, MTL_HybridDFCNN_highway_minibatch
...
def model_generation(model_architecture, model_hyperpara, train_hyperpara, data_info, classification_prob=False, data_list=None, tfInitParam=None, lifelong=False):
    ...
    if model_architecture == 'stl_cnn':
        if lifelong:
            ...
            learning_model = MTL_several_CNN_minibatch(dim_channels=cnn_channel_size, num_tasks=num_task, dim_fcs=fc_hidden_sizes, input_size=input_size, dim_kernel=cnn_kernel_size, dim_strides=cnn_kernel_stride, batch_size=batch_size, learning_rate=learning_rate, learning_rate_decay=learning_rate_decay, data_list=data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, skip_connect=skip_connect)
    elif model_architecture == 'dfcnn_singletask':
        print("Training STL-DFCNNs model (collection of DFCNN per a task)")
        ...
        if lifelong:
            ...
            learning_model = MTL_several_CNN_deconvtm_minibatch(num_task, cnn_channel_size, fc_hidden_size, input_size, cnn_kernel_size, cnn_kernel_stride, cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size, batch_size, learning_rate, learning_rate_decay, regularization_scale, data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, relation_activation_fn_cnn=None, skip_connect=skip_connect)
    elif model_architecture == 'dfcnn_tc2_singletask':
        print("Training STL-DFCNNs model (collection of DFCNN.tc2 per a task)")
        cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size = model_hyperpara['cnn_KB_sizes'], model_hyperpara['cnn_TS_sizes'], model_hyperpara['cnn_deconv_stride_sizes']
        if lifelong:
            raise NotImplementedError
        else:
            learning_model = MTL_several_CNN_deconvtm2_minibatch(num_task, cnn_channel_size, fc_hidden_size, input_size, cnn_kernel_size, cnn_kernel_stride, cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size, batch_size, learning_rate, learning_rate_decay, regularization_scale, data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, relation_activation_fn_cnn=None, skip_connect=skip_connect)
        ...
        if lifelong:
            ...
            tmp_cond = [fc_hidden_sizes[i][-1] == fc_hidden_sizes[i+1][-1] for i in range(num_task-1)]
            assert all(tmp_cond), "The output layer of tasks models have different numbers of units!"
            print("Training a single CNN model for all tasks - Multi-task Learning")
            learning_model = MTL_CNN_minibatch(dim_channels=cnn_channel_size, num_tasks=num_task, dim_fcs=fc_hidden_size, input_size=input_size, dim_kernel=cnn_kernel_size, dim_strides=cnn_kernel_stride, batch_size=batch_size, learning_rate=learning_rate, learning_rate_decay=learning_rate_decay, data_list=data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, skip_connect=skip_connect)
        ...
        if lifelong:
            ...
            learning_model = MTL_CNN_HPS_minibatch(dim_channels=cnn_channel_size, num_tasks=num_task, dim_fcs=fc_hidden_sizes[0], input_size=input_size, dim_kernel=cnn_kernel_size, dim_strides=cnn_kernel_stride, batch_size=batch_size, learning_rate=learning_rate, learning_rate_decay=learning_rate_decay, data_list=data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, skip_connect=skip_connect)
        ...
        if lifelong:
            ...
            learning_model = MTL_CNN_tensorfactor_minibatch(dim_channels=cnn_channel_size, num_tasks=num_task, dim_fcs=fc_hidden_sizes, input_size=input_size, dim_kernel=cnn_kernel_size, dim_strides=cnn_kernel_stride, batch_size=batch_size, learning_rate=learning_rate, learning_rate_decay=learning_rate_decay, data_list=data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, factor_type=factor_type, factor_eps_or_k=factor_eps_or_k, init_param=tfInitParam, skip_connect=skip_connect)
        ...
        if lifelong:
            ...
            raise NotImplementedError
    elif model_architecture == 'ewc_cnn':
        print("Training LL-CNN Elastic Weight Consolidation (EWC) model")
        if lifelong:
            raise NotImplementedError
        else:
            learning_model = MTL_CNN_EWC_minibatch(dim_channels=cnn_channel_size, num_tasks=num_task, dim_fcs=fc_hidden_size, input_size=input_size, dim_kernel=cnn_kernel_size, dim_strides=cnn_kernel_stride, batch_size=batch_size, learning_rate=learning_rate, learning_rate_decay=learning_rate_decay, data_list=data_list, fisher_multiplier=model_hyperpara['fisher_multiplier'], ewc_batch_size=model_hyperpara['ewc_batch_size'], padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, skip_connect=skip_connect)
    ...
    elif model_architecture == 'dfcnn_direct':
        cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size = model_hyperpara['cnn_KB_sizes'], model_hyperpara['cnn_TS_sizes'], model_hyperpara['cnn_deconv_stride_sizes']
        print("Training DF-CNN.direct model")
        if lifelong:
            raise NotImplementedError
        else:
            learning_model = MTL_DFCNN_direct_minibatch(num_task, cnn_channel_size, fc_hidden_size, input_size, cnn_kernel_size, cnn_kernel_stride, cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size, batch_size, learning_rate, learning_rate_decay, regularization_scale, data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, relation_activation_fn_cnn=None, skip_connect=skip_connect)
    elif model_architecture == 'dfcnn':
        cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size = model_hyperpara['cnn_KB_sizes'], model_hyperpara['cnn_TS_sizes'], model_hyperpara['cnn_deconv_stride_sizes']
        print("Training DF-CNN model")
        if lifelong:
            raise NotImplementedError
        else:
            learning_model = MTL_DFCNN_minibatch(num_task, cnn_channel_size, fc_hidden_size, input_size, cnn_kernel_size, cnn_kernel_stride, cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size, batch_size, learning_rate, learning_rate_decay, regularization_scale, data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, relation_activation_fn_cnn=None, skip_connect=skip_connect)
    elif model_architecture == 'dfcnn_tc2':
        cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size = model_hyperpara['cnn_KB_sizes'], model_hyperpara['cnn_TS_sizes'], model_hyperpara['cnn_deconv_stride_sizes']
        print("Training DF-CNN.tc2 model")
        if lifelong:
            raise NotImplementedError
        else:
            learning_model = MTL_DFCNN_tc2_minibatch(num_task, cnn_channel_size, fc_hidden_size, input_size, cnn_kernel_size, cnn_kernel_stride, cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size, batch_size, learning_rate, learning_rate_decay, regularization_scale, data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, relation_activation_fn_cnn=None, skip_connect=skip_connect)

    elif model_architecture == 'hybrid_dfcnn':
        cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size = model_hyperpara['cnn_KB_sizes'], model_hyperpara['cnn_TS_sizes'], model_hyperpara['cnn_deconv_stride_sizes']
        cnn_sharing = model_hyperpara['conv_sharing']
        if highway_connect > 0:
            print("\tHighway Connection - type %d" %(highway_connect))
            if lifelong:
                print("Training Hybrid DF-CNNs model (Highway conn.) - Lifelong Learning")
                raise NotImplementedError
            else:
                print("Training Hybrid DF-CNNs model (Highway conn.) - Multi-task Learning")
                learning_model = MTL_HybridDFCNN_highway_minibatch(num_task, cnn_channel_size, fc_hidden_size, input_size, cnn_kernel_size, cnn_kernel_stride, cnn_sharing, cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size, batch_size, learning_rate, learning_rate_decay, regularization_scale, data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, relation_activation_fn_cnn=None, highway_connect_type=highway_connect)
        else:
            ...
            if lifelong:
                ...
                print("Training Hybrid DF-CNNs model (ResNet skip-conn.) - Multi-task Learning")
                learning_model = MTL_HybridDFCNN_resnet_minibatch(num_task, cnn_channel_size, fc_hidden_sizes, input_size, cnn_kernel_size, cnn_kernel_stride, cnn_sharing, cnn_know_base_size, cnn_task_specific_size, cnn_deconv_stride_size, batch_size, learning_rate, learning_rate_decay, regularization_scale, data_list, padding_type=cnn_padding, max_pooling=cnn_pooling, dim_pool=cnn_pool_size, dropout=cnn_dropout, relation_activation_fn_cnn=None, skip_connect=skip_connect)
        ...
        if highway_connect > 0:
            print("\tHighway Connection - type %d" %(highway_connect))
            if lifelong:
                print("Training Hybrid DF-CNNs model (Highway conn./auto sharing/EM) - Lifelong Learning")
                raise NotImplementedError
            else:
                print("Training Hybrid DF-CNNs model (Highway conn./auto sharing/EM) - Multi-task Learning")
                raise NotImplementedError
        else:
            print("\tResNet Connection")
            if lifelong:
...
def train_mtl(model_architecture, model_hyperpara, train_hyperpara, dataset, data_type, classification_prob, useGPU=False, GPU_device=0, save_param=False, param_folder_path='saved_param', save_param_interval=100, save_graph=False, tfInitParam=None, run_cnt=0):
    ...
    if 'mnist' in data_type:
        ...
        num_task, num_train, num_valid, num_test, x_dim, y_dim, y_depth = omniglot_data_print_info(train_data, validation_data, test_data, True, print_info=False)
    elif 'officehome_incremental' in data_type:
        num_task, num_train, num_valid, num_test, x_dim, y_dim, y_depth = officehome_incremental_data_print_info(train_data, validation_data, test_data, True, print_info=False)
    ...
    with tf.Session(config=config) as sess:
        ...
        if train_hyperpara['mutual_info']:
            print("\nCompute Mutual Information of each conv layers")
            computed_forward_mutual_info, computed_backward_mutual_info = compute_mutual_info_convNet(test_data, learning_model, model_hyperpara, train_hyperpara, num_task, batch_size, y_depth, sess)
    ...
    if train_hyperpara['mutual_info']:
        result_summary['layerwise_mutual_info'] = computed_forward_mutual_info
        result_summary['layerwise_mutual_info_b'] = computed_backward_mutual_info
        ...
        with tf.Session(config=config) as sess:
            ...
            while learning_step < min(learning_step_max, epoch_bias + patience):
                ...
                if learning_step >= epoch_bias+min(patience, learning_step_max//len(task_training_order)):
                    ...
                    if train_hyperpara['confusion_matrix']:
                        print("\nCompute confusion matrix like table whose rows for true labels and columns for prediction")
                        confusion_matrices[task_for_train] = learning_model.compute_confusion_matrix(sess, train_data[task_for_train][0], train_data[task_for_train][1], task_for_train)
