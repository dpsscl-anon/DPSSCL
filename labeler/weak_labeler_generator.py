...
def generate_labeler_for_one_task(task_dir, task, data=None, task_similarity_score=None, task_similarity_score_threshold=1.0, sample_lf_from_earlier_task_prob=0.0, path_to_earlier_task_lf=None, lg_min=5, lg_max=25, lg_keep=10, return_snuba_iter_cnt=False, unified_score=False, finetune_transferred_wls=False):
    if data is None:
    ...
    if task_similarity_score is None or sample_lf_from_earlier_task_prob<=0.0 or path_to_earlier_task_lf is None:
        if return_snuba_iter_cnt:
            hfs, snuba_iter_cnt = lg.generate_snuba_lfs(log_file=os.path.join(task_parent_dir, 'log.txt'), return_snuba_iter_cnt=return_snuba_iter_cnt)
        else:
            hfs = lg.generate_snuba_lfs(log_file=os.path.join(task_parent_dir, 'log.txt'))
    else:
        print("\tUse pre-trained weak labelers")
        if return_snuba_iter_cnt:
            hfs, snuba_iter_cnt = lg.generate_snuba_lfs(log_file=os.path.join(task_parent_dir, 'log.txt'), sample_earlier_lfs=True, sample_earlier_lfs_prob=sample_lf_from_earlier_task_prob, path_earlier_lfs=path_to_earlier_task_lf, task_similarity_score=task_similarity_score, task_similarity_score_threshold=task_similarity_score_threshold, return_snuba_iter_cnt=return_snuba_iter_cnt, unified_score=unified_score, finetune_transferred_wls=finetune_transferred_wls)
        else:
            hfs = lg.generate_snuba_lfs(log_file=os.path.join(task_parent_dir, 'log.txt'), sample_earlier_lfs=True, sample_earlier_lfs_prob=sample_lf_from_earlier_task_prob, path_earlier_lfs=path_to_earlier_task_lf, task_similarity_score=task_similarity_score, task_similarity_score_threshold=task_similarity_score_threshold, unified_score=unified_score, finetune_transferred_wls=finetune_transferred_wls)
    ...
    return labelers_transferred_task_ids, labelers_converged_epoch_ratio, snuba_iter_cnt
