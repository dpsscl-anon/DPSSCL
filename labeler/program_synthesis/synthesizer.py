...
class Synthesizer(object):
    ...
    def generate_heuristics(self, model, max_cardinality=1, sample_earlier_lfs=False, sample_earlier_lfs_prob=0.5, task_similarity_score=None, task_similarity_score_threshold=0.0, path_earlier_lfs=None, unified_score=False, finetune_transferred_wls=False):
        """ 
            ...
            if unified_score and sample_earlier_lfs:
                ...
                for task_id, (earlier_lfs_dir) in enumerate(path_earlier_lfs):
                    for file_name in os.listdir(earlier_lfs_dir):
                        if '.pt' in file_name:
                            earlier_wls.append(self.fit_function(comb, 'cnn', model_file=os.path.join(earlier_lfs_dir, file_name), model_task_index=task_id, finetune_transferred_wls=False))
                ...
                for i in range(max_cardinality):
                    ...
                    beta_opt_temp = self.find_optimal_beta([earlier_wls[i]], self.val_primitive_matrix, feature_combinations_final[0], self.val_ground)
                    L_temp_val = self._apply_heuristics([earlier_wls[i]], self.val_primitive_matrix, feature_combinations_final[0], beta_opt_temp)
                ...
                for i in range(self.num_cnn_labelers):
                    if random() <= sample_earlier_lfs_prob and np.max(wl_sample_prob) > 0.0:
                        ...
                        if finetune_transferred_wls:
                            ...
                            heuristics.append(self.fit_function(comb, 'cnn', model_file=earlier_wls_filename[wl_index], model_task_index=earlier_wls_taskids[wl_index], finetune_transferred_wls=finetune_transferred_wls))
                        else:
                        ...
                        heuristics.append(self.fit_function(comb, 'cnn', model_file=None, model_task_index=-1, finetune_transferred_wls=finetune_transferred_wls))
            else:
                for i in range(self.num_cnn_labelers):
                    model_file, model_task_index = self._initialize_heuristics_function(sample_earlier_lfs=sample_earlier_lfs, sample_earlier_lfs_prob=sample_earlier_lfs_prob, task_similarity_score=task_similarity_score, task_similarity_score_threshold=task_similarity_score_threshold, path_earlier_lfs=path_earlier_lfs)
                    heuristics.append(self.fit_function(comb, 'cnn', model_file=model_file, model_task_index=model_task_index, finetune_transferred_wls=finetune_transferred_wls))
            heuristics_final.append(heuristics)

        return heuristics_final, feature_combinations_final

    def _initialize_heuristics_function(self, sample_earlier_lfs=False, sample_earlier_lfs_prob=0.5, task_similarity_score=None, task_similarity_score_threshold=0.0, path_earlier_lfs=None):
        ...
        if sample_earlier_lfs and random() <= sample_earlier_lfs_prob:
            ...
            if sum(transfer_score_above_th_tmp) > 1e-10:
                ...
                task_to_sample = np.random.choice(num_earlier_tasks, p=transfer_score_above_th)  # index in task list to sample
                ...
                if len(lf_file_name_list) > 0:
                    #print("\tUse pre-trained weak labelers")
                    sampled_lf_file_name = os.path.join(sampled_earlier_task, lf_file_name_list[randint(0, len(lf_file_name_list)-1)])
