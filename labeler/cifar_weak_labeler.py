...
class CifarWeakLabeler(torch.nn.Module):
    ...
    def train_cnn(self, X, y, verbose=False):
        ...
        for epoch in range(self.num_epochs):
            ...
            acc_list_for_convergence.append(correct2 / (total2 + 1e-10))
        for i, (acc_tmp) in enumerate(acc_list_for_convergence):
            if acc_tmp >= 0.95 * acc_list_for_convergence[-1]:
                self.converged_epoch_ratio = (i + 1) / len(acc_list_for_convergence)
                break
        return self
