...
def data_loader_binarytask_rep(path_to_data, num_classes, info_file_name, num_repetition, num_unlabel_data, num_label_data=-1, num_validation_data=-1, flatten_img=True, confidence_threshold=0.0):
    labeled_data_rep, unlabeled_data_rep = [], []
    for rep_cnt in range(num_repetition):
        data_file_name = info_file_name+'_rep'+str(rep_cnt)+'.pkl'
        labeled_data, unlabeled_data, test_data = data_loader_binarytask(path_to_data, num_classes, data_file_name, num_unlabel_data, num_label_data, num_validation_data=num_validation_data, flatten_img=flatten_img, confidence_threshold=confidence_threshold)
        labeled_data_rep.append(labeled_data)
        unlabeled_data_rep.append(unlabeled_data)
        print('Complete loading data for repetition %d'%(rep_cnt))
    return labeled_data_rep, unlabeled_data_rep, test_data


def data_loader_binarytask(path_to_data, num_classes, info_file_name, num_unlabel_data, num_label_data=-1, num_validation_data=-1, flatten_img=True, confidence_threshold=0.0):
    ...
    # sample instances randomly which exceed confidence threshold
    # return_list1 & num_selection : related to training set
    # return_list2 & num_selection2 : related to validation set
    ...
    if info_file_name in listdir(path_to_data):
        ...
        task_data_info, make_new_info_file = {'train': {}, 'validation': {}, 'labeled': {}}, True
    ...
    for class_cnt0 in range(num_classes-1):
        for class_cnt1 in range(class_cnt0+1, num_classes):
            ...
            # select labeled data
            start_index_second_class_l = index_of_second_class_binarytask(y_l)
            if make_new_info_file:
                num_l_class0, num_l_class1 = start_index_second_class_l, y_l.shape[0] - start_index_second_class_l
                if num_label_data < 0:  # handle default case: use all labeled data
                    num_label_data_used = y_l.shape[0]
                elif num_label_data > y_l.shape[0]:  # too large index out of bound
                    raise ValueError
                else:
                    num_label_data_used = num_label_data
                if str(class_cnt0) not in task_data_info['labeled'].keys():
                    index_l, _ = get_randomized_indices(num_l_class0, num_label_data_used // 2, 0, confidence_threshold)
                    task_data_info['labeled'][str(class_cnt0)] = index_l
                if str(class_cnt1) not in task_data_info['labeled'].keys():
                    index_l, _ = get_randomized_indices(num_l_class1, num_label_data_used // 2, 0, confidence_threshold)
                    task_data_info['labeled'][str(class_cnt1)] = index_l
            indices_labeled = list(task_data_info['labeled'][str(class_cnt0)]) + [a + start_index_second_class_l for a in task_data_info['labeled'][str(class_cnt1)]]
            labeled_x = x_l[indices_labeled]
            labeled_y = y_l[indices_labeled]

            # select unlabeled data, allow part of unlabeled data for validation
            start_index_second_class_u = index_of_second_class_binarytask(y_u)
            if make_new_info_file:
                num_u_class0, num_u_class1 = start_index_second_class_u, y_u.shape[0]-start_index_second_class_u
                if (str(class_cnt0) not in task_data_info['train'].keys()) or (str(class_cnt0) not in task_data_info['validation'].keys()):
                    index_u_train, index_valid = get_randomized_indices(num_u_class0, num_unlabel_data//2, max(num_validation_data//2, 30), confidence_threshold)
                ...
                if (str(class_cnt1) not in task_data_info['train'].keys()) or (str(class_cnt1) not in task_data_info['validation'].keys()):
                    index_u_train, index_valid = get_randomized_indices(num_u_class1, num_unlabel_data//2, max(num_validation_data//2, 30), confidence_threshold)
                    task_data_info['train'][str(class_cnt1)] = index_u_train
                    task_data_info['validation'][str(class_cnt1)] = index_valid

            indices_train_unlabeled = list(task_data_info['train'][str(class_cnt0)]) + [a+start_index_second_class_u for a in task_data_info['train'][str(class_cnt1)]]
            indices_valid_unlabeled = list(task_data_info['validation'][str(class_cnt0)]) + [a+start_index_second_class_u for a in task_data_info['validation'][str(class_cnt1)]]
            unlabeled_x = np.concatenate((x_u[indices_train_unlabeled], x_u[indices_valid_unlabeled]), axis=0)
            unlabeled_y = np.concatenate((y_u[indices_train_unlabeled], y_u[indices_valid_unlabeled]), axis=0)

            if flatten_img:
                num_labeled, num_unlabeled, num_test = labeled_x.shape[0], unlabeled_x.shape[0], x_test.shape[0]
                labeled_data.append( (labeled_x.reshape([num_labeled, -1]), labeled_y) )
                unlabeled_data.append( (unlabeled_x.reshape([num_unlabeled, -1]), unlabeled_y) )
                test_data.append( (x_test.reshape([num_test, -1]), y_test) )
            else:
                labeled_data.append( (labeled_x, labeled_y) )
                unlabeled_data.append( (unlabeled_x, unlabeled_y) )
...
def main():
    ...
    parser.add_argument('--data_label', help='Number of instances of labeled data for training, -1 means all', type=int, default=-1)  # new arg
    ...
    parser.add_argument('--save_result_in_pickle', help='Save result log in pickle file (in addition to mat file)', action='store_true', default=False)
    ...
    parser.add_argument('--mako_baseline', help='Baselines to compare against Mako labels (mv/rl/snorkel)', type=str, default='none')

    parser.add_argument('--mako_transfer_score_threshold', help='Threshold of transferability score to exclude less correlated tasks', type=float, default=1.0)
    parser.add_argument('--mako_transfer_knowledge_prob', help='Chance to use earlier tasks weak labelers instead of training them from scratch', type=float, default=0.0)
    ...
    data_num_label = args.data_label  # number of labeled data, from new argument
    ...
    # additional hyperparameters for Mako-CL integrated pipeline
    train_hyperpara['task_similarity_score_threshold'] = args.mako_transfer_score_threshold
    train_hyperpara['sample_lf_from_earlier_task_prob'] = args.mako_transfer_knowledge_prob
    train_hyperpara['strong_labeler'] = args.mako_baseline
    train_hyperpara['num_labeled_data'] = data_num_label
    ...
    if 'mnist_mako' in data_type: # added different number of labeled data
        ...
        train_hyperpara['num_train_valid_data_group'] = 5
        ...
        info_file_name = 'makointegrated_mnist_' + ('l' + str(data_num_label) if data_num_label > 0 else '')
        info_file_name = info_file_name + 'u' + (str(data_num_unlabel) if data_num_unlabel > 0 else str(0))
        train_data, validation_data, test_data = data_loader_binarytask_rep(getcwd() + '/Data/mako_labels/mnist_labels', 10, info_file_name, train_hyperpara['num_train_valid_data_group'], data_num_unlabel, num_label_data=data_num_label, confidence_threshold=label_conf_threshold, flatten_img=False)

        mnist_data_print_info(train_data, validation_data, test_data)
        classification_prob=True

    elif 'cifar10_mako' in data_type: # added different number of labeled data
        ...
        info_file_name = 'makointegrated_mnist_' + ('l' + str(data_num_label) if data_num_label > 0 else '')
        info_file_name = info_file_name + 'u' + (str(data_num_unlabel) if data_num_unlabel > 0 else str(0))
        train_data, validation_data, test_data = data_loader_binarytask_rep(getcwd() + '/Data/mako_labels/cifar10_labels', 10, info_file_name, train_hyperpara['num_train_valid_data_group'], data_num_unlabel, num_label_data=data_num_label, confidence_threshold=label_conf_threshold, flatten_img=False)
    ...
    train_hyperpara['mako_unlabel'] = data_num_unlabel

    ## Training the Model
    saved_result = train_run_for_each_model(model_architecture, model_hyperpara, train_hyperpara, [train_data, validation_data, test_data], data_type, mat_file_name, classification_prob, saved_result=None, useGPU=use_gpu, GPU_device=gpu_device_num, doLifelong=False, doSemiLifelong=True, saveParam=saveparam, saveParamDir=save_param_path, saveGraph=args.savegraph, tfInitParamPath=tensorfactor_param_path, task_order=train_task_order, debug_data_group=args.data_group, saveResultPkl=args.save_result_in_pickle)
